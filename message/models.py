...
class Client(models.Model):
    ...
    email = models.EmailField(
        unique=True, verbose_name="e-mail", help_text="Введите e-mail"
...
class Sms(models.Model):
    ...
    topic = models.CharField(
        max_length=100, verbose_name="Тема", help_text="Укажите тему письма"
    )  # Тема письма (строка)
...
class Mail(models.Model):
    ...
    status = models.CharField(
        ...
        default=STATUS_LAUNCHED,
        verbose_name="Статус рассылки",
    )  # Статус
    sms = models.ForeignKey(
        Sms, on_delete=models.CASCADE, related_name="mail", verbose_name="Сообщение"
    )  # Сообщение (внешний ключ на модель «Сообщение»)
    client = models.ManyToManyField(
        Client, verbose_name="Получатель"
